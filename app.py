...
def import_records_from_xlsx(file_bytes: bytes) -> list[dict]:
    ...
    for row_number, row in enumerate(rows, start=2):
        ...
        if len(result) >= MAX_IMPORT_ROWS:
            raise ValueError("El archivo supera el máximo local de 50.000 filas.")
        raw = {
...
def initialize_database(db_path: Path | str = DB_PATH, source_path: Path | str = SOURCE_PATH) -> None:
...
def period_bounds(conn: sqlite3.Connection, start: str | None, end: str | None) -> tuple[str, str]:
    ...
    start = start or limits["min_date"]
    end = end or limits["max_date"]
    try:
...
class TreasuryHandler(BaseHTTPRequestHandler):
    ...
    def handle_logout(self, conn: sqlite3.Connection):
        ...
        self.send_json(
            ...
            cookie="unach_session=; Path=/; Max-Age=0; HttpOnly; SameSite=Strict",
        )
